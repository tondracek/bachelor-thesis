...
%Volba použitých technologií pro vývoj navrhované aplikace je klíčovým krokem, který ovlivní nejen samotný proces vývoje, ale i konečnou kvalitu a použitelnost aplikace.
...
Vyvíjená aplikace bude využívat funkce jako je fotoaparát, galerie obrázků, aktuální poloha uživatele a mapa okolí uživatele.
Pro aplikaci bude vhodné mít možnost ji používat i mimo domov, například při cestování nebo návratu domů z cesty, kdy bude uživatel chtít najít obchody v~okolí své aktuální polohy.
Ve finálním kroku budou uživatelé většinou kontaktovat producenta pomocí kontaktních údajů, které budou dostupné v aplikaci u jednotlivých obchodů.
Přestože to není jediné řešení, budou uživatelé typicky k tomuto účelu používat svůj telefon.
Z těchto důvodů je nejvhodnější platformou pro vývoj aplikace mobilní aplikace.
Webová aplikace by sice zajistila větší dostupnost, ale primární využití je na mobilním telefonu, kde by webová aplikace ve srovnání s nativní mobilní aplikací poskytovala výrazně horší uživatelský zážitek.
V České republice je nejrozšířenějším operačním systémem pro mobilní zařízení Android, a proto byla jako cílová platforma zvolena Android mobilní aplikace.

\subsection{Programovací jazyk a UI framework} % TODO: mám rozdělit jazyk a UI framework?
% Možnosti pro Android vývoj
% - Java + XML - starší, ale stále používané
% - Kotlin + XML - moderní jazyk, ale starší způsob tvorby UI
% - Kotlin + Jetpack Compose - moderní a doporučovaný způsob tvorby UI
% Důvody volby Kotlinu a Jetpack Compose
% - Kotlin - moderní a doporučovaný jazyk pro Android
% - Jetpack Compose - moderní framework pro tvorbu UI
% - Deklarativní přístup k tvorbě UI
% - Jetpack Compose - méně kódu, jednodušší údržba, rychlejší vývoj, přirozené napojení na state management
...
% "Zvolené řešení Firebase představuje plnohodnotné backendové řešení typu Backend-as-a-Service"
