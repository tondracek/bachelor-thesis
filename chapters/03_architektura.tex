...
\begin{enumerate}
    ...
    \item Use-case provede doménovou logiku a v případě potřeby komunikuje s repozitářem.
    \item Repozitář získá nebo uloží data prostřednictvím datové vrstvy a vrátí výsledek ve formě DomainResult.
    \item Use-case zpracuje DomainResult a vrátí ho zpět ViewModelu.
    \item ViewModel zpracuje výsledek operace a aktualizuje stav obrazovky.
    \item Uživatelské rozhraní automaticky reaguje na změnu stavu a znovu se vykreslí.
\end{enumerate}

ViewModel vystavuje stav uživatelského rozhraní pomocí reaktivních datových toků (StateFlow), které uživatelské rozhraní průběžně pozoruje.
...
Hilt poskytuje automatickou správu životního cyklu objektů, integraci s ViewModely a snižuje množství boilerplate kódu potřebného pro konfiguraci závislostí.\footfullcite{android_hilt_overview}
...
Závislosti jsou definovány v samostatných konfiguračních modulech, které určují, jaké konkrétní implementace mají být v aplikaci použity.
...
Feature-based struktura projektu přispěla k lepší orientaci v kódu a omezení provázanosti mezi jednotlivými funkčními celky.
