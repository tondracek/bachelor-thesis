\chapter{Architektura kódu}\label{ch:architektura}
Před zahájením implementace je důležité navrhnout a naplánovat architekturu kódu.
Dobře navržená architektura pomůže ušetřit značné množství času a námahy v~budoucnu, například při přidávání nových funkcí nebo opravování stávajícího kódu.
Zároveň přispívá ke zvýšení jeho čitelnosti a udržovatelnosti.
Naopak špatně navržená architektura může vést k~problémům, jako je náročné zjišťování chyb a pomalejší vývoj.
...
Při návrhu byly převzaty vybrané principy běžně používané v~moderních architektonických přístupech, které jsou dále rozvedeny v~následujících podkapitolách.
...
Toto pojetí vychází z~principů doménově řízeného návrhu~\parencite{evans_ddd}.


\section{Architektonický vzor MVVM}\label{sec:mvvm}
Jako základní architektonický vzor byla zvolena architektura Model-View-ViewModel (MVVM)~\parencite{mvvm, android_mvvm}.
Jedná se o~doporučený vzor pro vývoj nativních Android aplikací, který je podporován samotným Android frameworkem a prosazován společností Google, která stojí za vývojem platformy Android~\parencite{android_recommended_architecture}.

Alternativami k~architektonickému vzoru MVVM jsou například Model-View-Presenter (MVP) nebo Model-View-Intent (MVI)~\parencite{mvp_mvvm_mvi}.
U~vzoru MVP Presenter přímo manipuluje s~uživatelským rozhraním, což není plně v~souladu deklarativním přístupem frameworku Jetpack Compose, který je využíván v~této aplikaci.
MVI je velmi dobře kompatibilní s~deklarativním přístupem frameworku Jetpack Compose, avšak jeho použití bývá spojeno s~vyšší komplexitou návrhu a správy stavu aplikace.
Některé jeho principy (jako například jednosměrný tok dat~\parencite{android_recommended_architecture} nebo \textit{Reducer pattern}~\parencite{redux_reducer_pattern}) jsou však využitelné i v~rámci MVVM vzoru a přispívají k~lepší předvídatelnosti toku dat v~aplikaci a byly při implementaci také využity (viz.\ sekce~\ref{subsec:ui-state-management}).
...
V~navrhované aplikaci jsou pro správu stavu obrazovek využívány asynchronní datové toky, do kterých při změně dat ViewModel publikuje nový stav jakožto neměnný (immutable) objekt~\parencite{compose_ui_architecture}.
View pak tento stav pozoruje a na jeho základě vykresluje uživatelské rozhraní.
View naopak předává uživatelské události (například kliknutí na tlačítko) ViewModelu, který na ně reaguje a případně aktualizuje stav nebo komunikuje s~Modelem.
Tento přístup je označován jako \textbf{jednosměrný tok dat} (unidirectional data flow)~\parencite{android_recommended_architecture} a přispívá k~lepší předvídatelnosti a stabilitě aplikace.
...
ViewModely v~této aplikaci nevykonávají žádné přímé akce nad uživatelským rozhraním, ale vystavují tzv. \textbf{vedlejší efekty} (side effects)~\parencite{android_side_effect}.
Efekty jsou jednorázové události, které vyvolávají akce v~uživatelském rozhraní, jako například navigace nebo zobrazení vyskakovacích oken.
View vystavované efekty interpretuje a na jejich základě provádí příslušné akce v~uživatelském rozhraní.

Jak bylo zmíněno výše, vzor MVVM je přímo podporovaný frameworkem Android.
ViewModely jsou v~Android frameworku tzv.\ lifecycle-aware, což znamená, že automaticky reagují na změny životního cyklu aplikace, například při změně konfigurace zařízení (otočení obrazovky) nebo při přechodu aplikace na pozadí.
Díky tomu nedochází ke ztrátě stavu uživatelského rozhraní a k~opakovanému načítání dat při těchto změnách~\parencite{android_viewmodel_overview}.
...
Projekt je strukturován podle funkčních celků (feature-based struktura), kdy jednotlivé části aplikace odpovídají konkrétním oblastem domény nebo uživatelské funkcionality~\parencite{android_modularization_patterns, martin_clean_architecture}.
...
\begin{itemize}
    ...
    \item \textbf{sample} -- ukázková data využívaná při testování a ladění aplikace.
\end{itemize}

Aplikace je rozdělena do několika hlavních funkčních celků.
Mezi nejdůležitější patří například:
\begin{itemize}
    \item \textbf{auth} -- registrace a přihlášení uživatelů,
    ...
    \item \textbf{core} -- logika jádra aplikace a architektonické abstrakce,
    \item \textbf{common} -- společné pomocné třídy a funkce.
\end{itemize}

Celek \textbf{shop} je založen především na entitě \textit{Shop} a s~ní související doménovou logiku.
...
Kromě toho složka ui obsahuje také podsložky \textbf{ui/common}, která obsahuje sdílené prvky uživatelského rozhraní (tlačítka, vstupní pole apod.), a \textbf{ui/core}, která obsahuje základní komponenty a centrální logiku pro práci s~uživatelským rozhraním (navigace a navigační lišta, vyvolávání vyskakovacích oken, informační lišty apod.).
...
Jedním ze základních principů architektury aplikace je oddělení doménové a datové vrstvy~\parencite{domain_data_layer_separation, evans_ddd, martin_clean_architecture}.
...
Konkrétní doménová logika je implementována v~use-case třídách, které představují hlavní vstupní bod pro její volání z~ViewModelů.
...
Díky definici rozhraní repozitářů v~doménové vrstvě a jejich implementaci v~datové vrstvě je závislost mezi těmito vrstvami jednosměrná a směřuje výhradně od datové vrstvy k~doménové vrstvě.
...
Díky tomu není nutné explicitně řídit aktualizaci uživatelského rozhraní -- změna stavu automaticky vede k~jeho opětovnému vykreslení.

Stav uživatelského rozhraní je v~aplikaci reprezentován neměnnými (immutable) datovými objekty~\parencite{compose_ui_architecture}.
...
\section{Vkládání závislostí}\label{sec:dependency_injection}
Pro řízení závislostí mezi jednotlivými částmi aplikace je využíván princip vkládání závislostí (Dependency Injection)~\parencite{android_dependency_injection}.
...
V~aplikaci je pro Dependency Injection použit framework Hilt, který je oficiálně doporučovaným řešením pro platformu Android~\parencite{android_recommend_hilt}.
Hilt poskytuje automatickou správu životního cyklu objektů, integraci s~ViewModely a snižuje množství pomocného (boilerplate) kódu potřebného pro konfiguraci závislostí~\parencite{android_hilt_overview}.
...
Architektura aplikace nevznikla v~konečné podobě od počátku vývoje, ale vyvíjela se postupně spolu s~narůstající komplexitou systému.
S~přibývajícími funkčními celky a třídami se ukázalo, že jasné oddělení odpovědností a modularizace projektu jsou nezbytné pro udržení kontrolovatelnosti kódu.
Zvolený návrh se v~praxi osvědčil při implementaci i následném rozšiřování aplikace a poskytuje stabilní základ pro její další vývoj.